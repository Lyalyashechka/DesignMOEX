...
```  
   ...
   * Количество запросов в сутки на получение актуальной котировки по паре:  2 785 000 * 600 = 1 671 000 000.
   ...
   |Теоретически возможный пиковый|696 720| 
...
## 4. Физическая схема.  
![hihgload_physic (1)](https://user-images.githubusercontent.com/77728314/168283637-36e1123e-459e-4310-93fd-86bb4e914ea9.jpg)
  ...
  ***4.1. Индексы.***  
  ...
  Рассмотрим подробнее [статистику по количеству сделок на акциях](https://www.moex.com/ru/marketdata/#/mode=groups&group=4&collection=3&boardgroup=57&data_type=history&date=2022-01-19&category=main), из нее видно, что основной объем торгов приходится на первые 10 активов (100 000 - 500 000 сделок в день). Каждый из этих активов будем мапить на свой физический сервер. Остальные активы будем мапить на физические сервера по принципу, чтобы на них приходилось 250 000 сделок в день. На первые 10 активов приходится ~ 1 400 000 сделок в день, тогда на остальные 2 785 000 - 1 400 000 = 1 375 000. Тогда нам понадобится 10 + 1 375 000 / 250 000 = 16 серверов для каждой базы данных.   
## 5. Технологии.  
...
|MemSQL|Таблицы ордеров, активов пользователя, хранение временных рядов истории торгов| MemSQL позиционирует себе как новый класс БД HTAP (Hybrid transactional / analytical processing). MemSQL обходит узкое место ввода-вывода большинства алтернативных БД, помещая данные в память и переводя SQL на C++, что позволяет базе данных отправлять больше транзакций в секунду с чрезвычайно низкой задержкой (<1 мс на запрос). MemSQL совместим с MySQL и для работы с ним можно использовать те же инструменты и драйверы, что и для MySQL. Также одним из преимуществ MemSQL является предоставление аналитики в режиме реального времени. MemSQL способен обрабатывать 80 000 запросов в секунду [источник](https://www.gavick.com/blog/memsql-80-000-queries-per-second) - такой производительности точно хватит для наших серверов.|  
...
## 6. Схема проекта. 
![Untitled Diagram](https://user-images.githubusercontent.com/77728314/168284732-804eebf2-6ae9-4400-8e60-652bc06272fa.jpg)
...
* *simple_data_api* отвечает за общую информацию об активах и парах на бирже.  
* *trade_api* отвечает за оформление и просмотр ордеров на рынках. Когда на сервер поступает запрос на оформление ордера, этот сервис делает запрос в базу данных ордеров, проверяя, может ли он быть исполнен. В случае если запрос не может быть исполнен сейчас, кладет его в базу данных, в противном - за одну транзакцию переводит все ордера, которые могут быть исполнены в состояния done (или изменяет доступное количество) и кладет поступивший ордер в базу со статусом done. 
* *trade_history_worker* раз в минуту запрашивает у таблицы ордеров минимальную, максимальную, среднюю цену исполненных ордеров по каждой паре и заносит эту информацию в таблицу истории торгов.  

## 7. Список серверов.
* NGINX   
Количество RPS nginx на 36 ядерном процессоре для запрососов 1кб - 1 309 358 [источник](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/), для задач биржи хватит 1 такого сервера. Сетевой карты хватит с пропускной способностью 10Гбит/с. 
* trade_api  
...
* trade_history_worker 
Раз в минуту должен выполнить ~4k запроса на получение информации от таблицы ордеров и записать эти данные в таблицу истории торгов. Хватит 1 производительного сервера.
* MemSQL  
Исходя из пункта 4.2 нам понадобится по 16 серверов для каждой таблички MemSql с высокой производительностью. 
* PostgreSQL  
Хранит информацию о 4 000 активах и парах. Это примерно ```15Гб```. Хватит 1 сервера. Запросы к нему не подвержены высокой нагрузке. 
...
|--------|----------|------------|-----|----------|  
|NGINX   |64        |64          |SSD 512|1 + 1 |
|trade_api|64        |64         |SSD 512|16 + 16  |
